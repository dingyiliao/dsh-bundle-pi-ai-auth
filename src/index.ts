...
interface InteractionState {
  latest?: AuthorizationNotice
  readonly openedUrls: Set<string>
  readonly transient: Set<{ controller: AbortController; settled: Promise<void> }>
}

interface ProviderLayers {
...
function providerLayers(ctx: Context, provider: string): ProviderLayers {
...
function providerId(entry: AuthorizationEntry): string | undefined {
  const key = String(entry.key)
  if (!key.startsWith(KEY_PREFIX)) return undefined
  const provider = key.slice(KEY_PREFIX.length)
  if (provider.length === 0 || provider.includes('/')) return undefined
  return entry.methods.some(method => method.id === 'oauth') ? provider : undefined
}

function oauthEntries(ctx: Context): Array<{ provider: string; entry: AuthorizationEntry }> {
...
function entryFor(ctx: Context, provider: string): AuthorizationEntry | undefined {
...
function combinedSignal(invocation: CommandInvocation, prompt: AuthorizationPrompt): AbortSignal {
  return prompt.signal === undefined
    ? invocation.signal
    : AbortSignal.any([invocation.signal, prompt.signal])
}

function promptOptions(prompt: AuthorizationPrompt): AskUserQuestionOption[] | undefined {
  if (prompt.kind !== 'select') return undefined
...
function promptDetail(state: InteractionState, prompt: AuthorizationPrompt): string | undefined {
  const parts: string[] = []
  const notice = state.latest
  if (notice !== undefined && notice.message !== prompt.message) parts.push(notice.message)
  const url = httpUrl(notice?.url)
  if (url !== undefined) parts.push(`[打开登录页面](${url})`)
  if (notice?.code !== undefined) parts.push(`授权码：\`${notice.code.replaceAll('`', "'")}\``)
...
function openNoticeUrl(ctx: Context, state: InteractionState, notice: AuthorizationNotice): void {
  const url = httpUrl(notice.url)
  if (url === undefined || state.openedUrls.has(url)) return
...
async function askPrompt(
  ...
  try {
    ...
    if (prompt.kind !== 'select') return value
    const selected = prompt.options.find(option => option.label === value)?.id
      ?? prompt.options.find(option => option.id === value)?.id
    if (selected === undefined) throw new Error('authorization method is not recognized')
    return selected
  } catch (error: unknown) {
...
function showCodeNotice(
  ...
  const url = httpUrl(notice.url)
  if (url === undefined || notice.code === undefined) return
  ...
  const handle = {
    ...
    settled: ctx.userQuestions.ask({
      ...
      if (!signal.aborted) ctx.logger.warn(`pi-ai-auth: device-code notice failed: ${safeError(error)}`)
    }),
  }
...
async function chooseProvider(
  ctx: Context,
  invocation: CommandInvocation,
  entries: Array<{ provider: string; entry: AuthorizationEntry }>,
  action: string,
): Promise<{ provider: string; entry: AuthorizationEntry } | undefined> {
...
async function resolveTarget(
  ...
  candidates: Array<{ provider: string; entry: AuthorizationEntry }>,
  action: string,
): Promise<{ target?: { provider: string; entry: AuthorizationEntry }; error?: CommandResult }> {
...
async function list(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  if (splitInput(invocation.rawInput).length > 0) return { kind: 'error', text: '用法：/auth-list' }
  const entries = oauthEntries(ctx)
  if (entries.length === 0) return { kind: 'error', text: '当前 DSH 没有注册支持 OAuth 的 pi-ai Provider。' }
  const rows = await Promise.all(entries.map(async ({ provider, entry }) => {
    const layers = providerLayers(ctx, provider)
    if (!layers.enabled) return `- ${entry.label}（${provider}）：未添加`
...
async function add(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  ...
  if (target === undefined) return { kind: 'error', text: '没有可添加的 OAuth Provider。' }
...
async function login(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  ...
  if (resolved.error !== undefined) return resolved.error
  const target = resolved.target
  if (target === undefined) return { kind: 'error', text: '没有已启用的 OAuth Provider。' }
  if (!providerLayers(ctx, target.provider).enabled) {
...
async function status(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  ...
  if (extra !== undefined) return { kind: 'error', text: '用法：/auth-status [provider]' }
  if (rawProvider === undefined) return list(ctx, invocation)
  const entry = entryFor(ctx, rawProvider)
  if (entry === undefined) return { kind: 'error', text: `Provider ${JSON.stringify(rawProvider)} 没有可用的 OAuth 授权流程。` }
  ...
  if (entry.inFlight) return { kind: 'success', text: `${entry.label}（${rawProvider}）：正在登录。` }
...
async function logout(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  const [rawProvider, extra] = splitInput(invocation.rawInput)
  if (extra !== undefined) return { kind: 'error', text: '用法：/auth-logout [provider]' }
  const candidates = oauthEntries(ctx).filter(candidate => providerLayers(ctx, candidate.provider).enabled)
...
async function remove(ctx: Context, invocation: CommandInvocation): Promise<CommandResult> {
  const [rawProvider, extra] = splitInput(invocation.rawInput)
  if (extra !== undefined) return { kind: 'error', text: '用法：/auth-remove [provider]' }
  const candidates = oauthEntries(ctx).filter(candidate => providerLayers(ctx, candidate.provider).enabled)
  ...
  if (!layers.enabled) return { kind: 'error', text: `${target.provider} 尚未启用。` }
  if (layers.base) {
    return {
      kind: 'error',
  ...
  if (!layers.user) return { kind: 'error', text: `${target.provider} 不属于可移除的用户配置。` }
  ctx.authorization.cancel(target.entry.key)
  try {
    await ctx.credentials.deleteRecord(keyFor(target.provider))
    await ctx.settings.mutate(SETTINGS_NS, [{ op: 'unset', path: ['providers', target.provider] }])
