	...
	factory: (require) => {
		...
		function PiAiAuthCard({ provider, configured, authorization, t }) {
			...
			const login = (0, react.useCallback)(async () => {
				...
				setError(void 0);
				try {
					for await (const frame of authorization.begin(key, controller.signal)) if (frame.type === "notice") setNotice({
					...
					else if (frame.type === "failed") setError(t("loginFailed", { message: frame.message }));
