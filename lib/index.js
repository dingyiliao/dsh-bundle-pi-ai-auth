...
function requestedMethod(raw, offered) {
    const value = raw ?? 'oauth';
    if (offered.some(method => method.id === value))
        return value;
...
async function runLogin(ctx, invocation, provider, entry, rawMethod) {
    if (entry.inFlight) {
...
async function add(ctx, invocation) {
    ...
    if (extra !== undefined)
        return { kind: 'error', text: '用法：/auth-add [provider] [method]' };
...
async function login(ctx, invocation) {
    const [rawProvider, rawMethod, extra] = splitInput(invocation.rawInput);
    if (extra !== undefined)
        return { kind: 'error', text: '用法：/auth-login [provider] [method]' };
    const candidates = oauthEntries(ctx).filter(candidate => providerLayers(ctx, candidate.provider).enabled);
...
export function apply(ctx) {
    ...
    const definitions = [
        ...
        { name: 'auth-add', description: '添加 Provider 并完成 OAuth 登录', input: { hint: '[provider] [method]' }, handler: (value) => add(ctx, value) },
        { name: 'auth-login', description: '登录一个已启用的 OAuth Provider', input: { hint: '[provider] [method]' }, handler: (value) => login(ctx, value) },
